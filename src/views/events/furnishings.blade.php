...
@section('current', trans('entities.furnishings') . ' ' . $stand_name)

@section('button')
    <button type="button" id="modalPaymentLabel" class="btn btn-success text-uppercase" data-bs-toggle="modal"
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card card-tabs">
                <div class="card-body table-responsive p-3">
                    <ul class="nav nav-tabs card-header-tabs mb-2" role="tablist">
                        @foreach ($modules as $key => $module)
                            <li class="nav-item">
                                <button type="button" class="nav-link {{ $key == 0 ? 'active' : '' }}"
                                    id="tab-{{ $module->id }}" data-bs-toggle="tab" role="tab"
                                    data-bs-target="#tab--{{ $module->id }}" aria-controls="tab--{{ $module->id }}"
                                    aria-selected="{{ $key == 0 ? 'true' : 'false' }}">{{ $module->code }}</button>
                            </li>
                        @endforeach
                    </ul>
                    <div class="tab-content" id="custom-tabs-one-tabContent">
                        @foreach ($modules as $key => $module)
                            <div class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}" id="tab--{{ $module->id }}"
                                role="tabpanel" aria-labelledby="tab-{{ $module->id }}">
                                <table class="table table-hover text-nowrap" data-module-id="{{ $module->id }}">
                                    <thead>
                                        <tr>
                                            <th>{{ trans('tables.description') }}</th>
                                    ...
                                    <tbody>
                                        @foreach ($list as $l)
                                            ...
                                            {{-- <tr data-id="{{$l->id}}" class="{{$l->is_supplied && $l->min > 0 ? 'bg-info' : ''}}"> --}}
                                            <tr data-id="{{ $l->id }}" class="{{ $class }}"
                                                data-extra-price="{{ $l->extra_price }}" data-max="{{ $l->max }}">
                                                <td>{{ $l->description }}</td>
                                                <td data-value="{{ $l->is_supplied }}" name="is_supplied">
                                                    {{ $l->is_supplied ? trans('generals.yes') : trans('generals.no') }}
                                                </td>
                                                ...
                                                <td>
                                                    <a href="javascript:void(0);" id="modalImgLabel"
                                                        onclick="assignImg(this)" role="button" data-bs-toggle="modal"
                                                        data-bs-target="#modalImg"><img
                                                            src="{{ asset('img/furnishings/' . $l->file_path) }}"
                                                            class="table-img h-px-150"></a>
                                                </td>
                                                <td class="text-lg text-bold"><span data-total-partial=""></span> €</td>
                                            </tr>
                                        @endforeach
                ...
                <div class="card-body">
                    <p class="mb-3 text-end p-3 text-lg">
                        <strong>{{ trans('generals.total') }}</strong>: <span data-total></span>€
                        <br>
                    ...
                    {{-- <button type="button" id="modalPaymentLabel" class="btn btn-success text-uppercase"
